<template>
    ...
    <header class="top-deck">
        ...
        <div class="category-wrapper">
            <nav class="category-nav" ref="navRef" @wheel="handleWheel" @mousedown="handleMouseDown"
                @mousemove="handleMouseMove" @mouseup="handleMouseUp" @mouseleave="handleMouseUp">
                <button v-for="cat in categories" :key="cat.type" class="nav-item"
                    :class="{ active: currentCategory === cat.type }" @click="selectCategory(cat.type)">
                    {{ cat.name }}
                </button>
            </nav>
        </div>
...
@media (max-width: 950px) {
    ...
    .stat-label {
        /* 使用这种方式隐藏，可以保持 DOM 占位，且能做平滑过渡（可选） */
        display: none;
    }
